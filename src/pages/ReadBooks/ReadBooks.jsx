...
import { deleteReadFromLocalStorage } from '../../Utility/AddToLocalStorage';

const ReadBooks = ({book,handleDeleteRead}) => {

   const {image,bookName,author,rating, bookId,totalPages } = book;

   const handleReadDelete=(id)=>{
      handleDeleteRead(id)
      deleteReadFromLocalStorage(id)
   }
        ...
        <div className='flex justify-center pageFont p-2'>
            <div className='w-full bg-base-100 rounded-2xl shadow-md p-4 border border-gray-200'>
                ...
                <div className='flex justify-between gap-5'>
                    <Link to={`/bookdetails/${bookId}`} className='btn bg-teal-400 hover:bg-teal-500  text-white'>Detalis</Link>
                    <button onClick={()=>handleReadDelete(bookId)} className='btn bg-red-400 hover:bg-red-500  text-white'>Delete</button>
                </div>
